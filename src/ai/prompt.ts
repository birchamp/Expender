...
export function buildUserPrompt(trip: Trip, receiptCount: number): string {
  ...
  const lines = [
    ...
    '',
    'Use the trip context to write business_purpose, and to disambiguate the currency and the merchant when the print is unclear. Do not let the trip context override anything legible on the receipt itself.',
  ];
